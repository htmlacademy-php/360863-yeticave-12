    ...
    <form class="form form--add-lot container form--invalid" action="../add.php" method="post"
        ...
        <div class="form__container-two">
            <div class="form__item <?= !empty($errors['lot-name'])? 'form__item--invalid' : '' ?>"> <!-- form__item--invalid -->
                ...
                <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота"
                       value="<?= !empty($safeData['lot-name'])? $safeData['lot-name'] : ''?>">
        ...
        <div class="form__item form__item--wide <?= !empty($errors['message'])? 'form__item--invalid' : '' ?>">
            ...
            <textarea id="message" name="message"
                      placeholder="Напишите описание лота"><?= !empty($safeData['message'])? $safeData['message'] : '' ?></textarea>
        ...
        <div class="form__item form__item--file <?= !empty($errors['lot-img'])? 'form__item--invalid' : ''?>">
            ...
            <div class="form__input-file">
                ...
                <input type="hidden" name="img" value="<?= !empty($imgValue)? $imgValue : ''?>"/>
                <label for="lot-img">
                    Добавить
                </label>
            </div>
        ...
        <div class="form__container-three">
            <div class="form__item form__item--small <?= !empty($errors['lot-rate'])? 'form__item--invalid' : ''?>">
                ...
                <input id="lot-rate" type="text" name="lot-rate" placeholder="0"
                       value="<?= !empty($safeData['lot-rate'])? $safeData['lot-rate'] : ''?>">
            ...
            <div class="form__item form__item--small <?= !empty($errors['lot-step'])? 'form__item--invalid' : ''?>">
                ...
                <input id="lot-step" type="text" name="lot-step" placeholder="0"
                       value="<?= !empty($safeData['lot-step'])? $safeData['lot-step'] : ''?>">
            ...
            <div class="form__item <?= !empty($errors['lot-date'])? 'form__item--invalid' : ''?>">
                ...
                <input class="form__input-date" id="lot-date" type="text" name="lot-date"
                       placeholder="Введите дату в формате ГГГГ-ММ-ДД" value="<?= !empty($safeData['lot-date'])? $safeData['lot-date'] : ''?>">
