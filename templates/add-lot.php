    ...
    <form class="form form--add-lot container form--invalid" action="../add.php" method="post"
        ...
        <div class="form__container-two">
            <div class="form__item <?php if (!empty($errors['lot-name'])) {
                ...
                <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота"
                       value="<?php if (!empty($safeData['lot-name'])) {
                           echo $safeData['lot-name'];
                       } ?>">
        ...
        <div class="form__item form__item--wide <?php if (!empty($errors['message'])) {
            ...
            <textarea id="message" name="message"
                      placeholder="Напишите описание лота"><?php if (!empty($safeData['message'])) {
                    echo $safeData['message'];
                } ?></textarea>
            <?php if (!empty($errors['message'])): ?>
                <span class="form__error"><?= $errors['message']; ?></span>
            <?php endif; ?>
        </div>
        <div class="form__item form__item--file <?php if (!empty($errors['lot-img'])) {
            echo 'form__item--invalid';
        } ?>">
            <label>Изображение <sup>*</sup></label>
            <div class="form__input-file">
                <input class="visually-hidden" type="file" name="lot-img" id="lot-img" value="">
                <input type="hidden" name="img" value="<?php if (!empty($imgValue)) {
                    echo $imgValue;
                } ?>"/>
                <label for="lot-img">
                    Добавить
        ...
        <div class="form__container-three">
            <div class="form__item form__item--small <?php if (!empty($errors['lot-rate'])) {
                ...
                <input id="lot-rate" type="text" name="lot-rate" placeholder="0"
                       value="<?php if (!empty($safeData['lot-rate'])) {
                           echo $safeData['lot-rate'];
                       } ?>">
            ...
            <div class="form__item form__item--small <?php if (!empty($errors['lot-step'])) {
                ...
                <input id="lot-step" type="text" name="lot-step" placeholder="0"
                       value="<?php if (!empty($safeData['lot-step'])) {
                           echo $safeData['lot-step'];
                       } ?>">
            ...
            <div class="form__item <?php if (!empty($errors['lot-date'])) {
                ...
                <input class="form__input-date" id="lot-date" type="text" name="lot-date"
                       placeholder="Введите дату в формате ГГГГ-ММ-ДД" value="<?php if (!empty($safeData['lot-date'])) {
                    echo $safeData['lot-date'];
                } ?>">
